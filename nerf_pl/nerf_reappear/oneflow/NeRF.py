...
class NeRF(nn.Module):
    ...
    def forward(self, x, sigma_only=False):
        ...
        if not sigma_only:
            input_xyz, input_dir = \
                flow.split(x, [self.in_channels_xyz, self.in_channels_dir], dim=-1)
        else:
            input_xyz = x
...
# model=NeRF()
# input=flow.linspace(0,10,90*2).view(2,90)
# output=model(input)
# print(input,output)
...
#           9.9441, 10.0000]], dtype=oneflow.float32) tensor([[1.0000e+00, 1.0000e+00, 1.0000e+00, 2.0499e+03],
#         [1.0000e+00, 1.0000e+00, 1.0000e+00, 8.0291e+03]],
#        dtype=oneflow.float32, grad_fn=<concat_backward>)